...
onMounted(() => {
  // Observador único y robusto para (re)cargar personajes y (re)suscribir a RLS.
  // Se apoya en el estado `isReady` del store para no ejecutarse hasta que la
  // sesión se haya hidratado.
  watch(
    () => ({ ready: auth.isReady, uid: auth.user?.id }),
    async (newState, oldState) => {
      const { ready, uid } = newState

      // 1. Store listo y hay usuario: Cargar datos y suscribir a canal.
      if (ready && uid) {
        // Evitar recargas si solo cambió `isReady` pero el usuario es el mismo
        if (uid !== oldState?.uid) {
          await loadCharacters()
        }

        // Resuscripción al canal si el ID de usuario cambió.
        if (uid !== oldState?.uid && channel.value?.isSubscribed()) {
          await channel.value.unsubscribe()
        }
        if (!channel.value || !channel.value.isSubscribed()) {
      ...
      // 2. Store listo pero sin usuario: Limpiar y desuscribir.
      if (ready && !uid) {
        loading.value = false
        characters.value = []
        if (channel.value) {
