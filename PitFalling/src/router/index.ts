...
router.beforeEach(async (to) => {
  ...
  // Asegura que la sesión esté hidratada una vez. Aumentamos el timeout para
  // dar margen en conexiones lentas o si Supabase está frío.
  if (!auth.isReady && !justLoggedOutRecently) {
    try {
      await Promise.race([
        auth.loadSession(),
        new Promise(resolve => setTimeout(resolve, 2500)) // Timeout extendido
      ])
    } catch (e) {
      console.error('[guard] loadSession failed', e)
      // En caso de fallo de red, evitamos un bucle infinito en rutas protegidas
      if (to.meta.requiresAuth) {
        return false // Cancela la navegación actual
      }
    }
  }
